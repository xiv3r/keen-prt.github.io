# Xiaomi Router 3P <YezBadge type="keenetic" text="5.1.4" url="/assets/files/firmware/Xiaomi-R3P.7z"/>
...
1. **Запуск скрипта.** В архиве запустите `!Start.bat` в папке `Установка KeenBOOT`.
2. **Выбор пунктов.** Поочерёдно выберите пункты `1 → 2 → 3 → 4`.

   ![Меню установщика Breed](/assets/images/wiki/guides/Xiaomi/install.png)

3. **Резервная копия и переход в KeenBOOT.** Скрипт сохранит резервную копию раздела `factory` (EEPROM) в папку `installer/data` и перезагрузит устройство в загрузчик KeenBOOT.
4. **Вход в KeenBOOT.** Перейдите в загрузчик KeenBOOT ([как?](/wiki/helpful/keenboot#вход-в-загрузчик)) по адресу `192.168.1.1`.
5. **Резервная копия.** Сохраните резервную копию `Full` на случай отката прошивки.

6. **Подготовка EEPROM.** Используйте EEPROM, созданный скриптом при установке со стоковой прошивки, либо резервную копию EEPROM из Breed, если Keenetic был установлен ранее. В [инструменте «Внести MAC»](/wiki/helpful/tools#converter) загрузите EEPROM, укажите MAC-адрес с наклейки и поместите созданный `eeprom.bin` в папку с прошивкой.

### Способ #1 <Badge type="keenetic" text="Новый, рекомендуемый" />

::: danger
Если сейчас установлена версия KeeneticOS не из этого проекта, перед сменой загрузчика выполните команду через Telnet в Breed:
```shell
flash erase 0x80000 0xff80000
```
:::

1. Поочерёдно загрузите из архива `Firmware`, `EEPROM` и `U-Config` через веб-интерфейс, следуя [инструкции](/wiki/helpful/keenboot#установка-прошивки).
...
1. **Раздача файлов.** Запустите `!HFS.exe` и перетащите в его окно все `.bin`-файлы из папки с прошивкой: `U-Config`, `U-State`, `EEPROM` и `firmware_Xiaomi-R3P_xxx`.
