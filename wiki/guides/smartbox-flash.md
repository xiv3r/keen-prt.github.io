# SmartBox Flash <OTABadge type="keenetic" text="5.1.4 (OTA)"/> <YezBadge type="keenetic" text="5.0.12" url="/assets/files/firmware/SmartBox-Flash.7z"/>
...
![SmartBox Flash](/assets/images/wiki/guides/SmartBox/flash.png)
...
1. **Запуск скрипта.** Скачайте архив и запустите `Start_Flash.bat`. Скрипт сохранит резервную копию EEPROM, установит загрузчик Breed и перезагрузит роутер.
...
2. Поочерёдно загрузите из архива `Firmware`, `EEPROM` и `U-Config` через веб-интерфейс, следуя [инструкции](/wiki/helpful/keenboot#установка-прошивки).
...
Вместо резистора R85 можно использовать свободную площадку R82, однако следует учесть, что на некоторых партиях плата может быть некачественной, и площадка может легко оторваться. Рекомендуем использовать R85. Также вместо резистора R1389 средний вывод конвертера является не менее актуальной альтернативой.
:::

![USB-Mod SmartBox Flash, фото 1](/assets/images/wiki/helpful/usb-mod/flash-1.jpg){width=300px height=100px}

![USB-Mod SmartBox Flash, фото 2](/assets/images/wiki/helpful/usb-mod/flash-2.jpg){width=300px height=100px}

![USB-Mod SmartBox Flash, фото 3](/assets/images/wiki/helpful/usb-mod/flash-3.jpg){width=300px height=100px}
